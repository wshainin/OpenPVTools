package.path = package.path .. ";" .. os.getenv("HOME") .. "/workspace/OpenPV/parameterWrapper/PVModule.lua"
local pv = require "PVModule"
debugParsing                       = true

--HyPerLCA parameters		   
local VThresh                      = 0.0125 -- 0.025
local VWidth                       = 0 --infinity
local dWMax                        = 1
local dWMaxSLP                     = 1.0
...
--local zX                           = 3  -- zX = 0 1 2 
--local zY                           = 2  -- zY = 0 1 2 
local strideX                      = 4 --patchSizeX/math.pow(2,zX) -- divide patchSize by 2^z to obtain dictionaries that are (2^z)^2 Xs overcomplete
local strideY                      = 4 --patchSizeY/math.pow(2,zY) -- 

local overcompleteness             = 28 --math.pow(2,zX) * math.pow(2,zY)--math.pow(math.pow(2,z),2) --(2^z)^2
local numBasisVectors              = overcompleteness * strideX * strideY * 3 * 2; -- nf = overcompleteness x (stride X) x (Stride Y) * (# color channels) * (2 if rectified) 
local S1_numFeatures               = numBasisVectors --patchSizeX * patchSizeY * nf_Image * 2 -- (patchSize/stride)^2 Xs overcomplete (i.e. complete for orthonormal ICA basis for stride == patchSize)
...
local experimentName               = "ImageNetVID_128X72_S1X" .. overcompleteness .. "_" .. patchSizeX .. "X" .. patchSizeY .. "_" .. temporalKernelSize .. "X" .. numFrames .. "frames" 
local runName                      = "train_8batch" 
local runVersion                   = 2
local machinePath                  = "/home/wshainin/workspace" 
local databasePath                 = "imageNet/VID128x72" 
local outputPath                   = machinePath .. "/" .. databasePath .. "/" .. experimentName .. "/" .. runName .. runVersion
local inputPath                    = nil --machinePath .. "/" .. databasePath .. "/" .. experimentName .. "/" .. runName .. runVersion-1  
local inputPathSLP                 = nil --machinePath .. "/" .. databasePath .. "/" .. experimentName .. "/" .. runName .. runVersion-1
local numImages                    = 1122397
local displayPeriod                = 900
local numEpochs                    = 1
local stopTime                     = numImages * displayPeriod * numEpochs
local checkpointID                 = nil --"0018000" --stopTime-- 
...
local dtScaleMax                   = 0.0011 --0.11   -- minimum value for the maximum time scale, regardless of tau_eff
local dtScaleMin                   = 0.001 --0.1    -- default time scale to use after image flips or when something is wacky
            ...
            if (delta_frame >= 0 and delta_frame < temporalKernelSize) then
                ...
                if i_delay == 1 then -- the first delay layer stores the original connections
                    ...
                    "S1_" .. 0 .. "To" .. "Frame" .. delta_frame .. "ReconS1Error",
                    {
                        ...
                        receiveGpu                          = false;
                        sharedWeights                       = true;
                        weightInitType                      = "UniformRandomWeight"; --"FileWeight"; 
