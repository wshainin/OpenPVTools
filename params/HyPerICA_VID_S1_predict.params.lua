...
local plasticityFlagPredict        = true --false
...
local experimentName               = "ImageNetVID_128X72_S1X" .. overcompleteness .. "_" .. patchSizeX .. "X" .. patchSizeY .. "_" .. temporalKernelSize .. "X" .. numFrames .. "frames" .. "_predict"
local experimentNameTmp            = "ImageNetVid_2015_vehicles_S1X" .. overcompleteness .. "_" .. patchSizeX .. "X" .. patchSizeY .. "_" .. temporalKernelSize .. "X" .. numFramesTmp .. "frames"
local experimentNameTmp2           = "ImageNetVid_S1X" .. overcompleteness .. "_" .. patchSizeX .. "X" .. patchSizeY .. "_" .. temporalKernelSize .. "X" .. numFramesTmp .. "frames"
local runName                      = "train" 
local runVersion                   = 6
local runVersionTmp                = 6
local machinePath                  = "/home/gkenyon" -- "/home/ec2-user/mountData" --"/nh/compneuro/Data" --"/Volumes/mountData" --
...
local inputPathPredict             = machinePath .. "/" .. databasePath .. "/" .. experimentName .. "/" .. runName .. runVersion-1
local numImages                    = 10035 --8211--
local displayPeriod                = 900 --1200
local numEpochs                    = 1
local stopTime                     = numImages * displayPeriod * numEpochs
local checkpointID                 = "1287000" --stopTime-- 
local checkpointID_SLP             = "1287000" --stopTime--
local checkpointID_predict         = "1287000" --stopTime-- 
...
--i/o parameters
local imageListPath                = machinePath .. "/" .. databasePath .. "/" .. "imageNetVID_128x72_landscape_list.txt" 
...
local dtScaleMax                   = 0.11   -- minimum value for the maximum time scale, regardless of tau_eff
local dtScaleMin                   = 0.1    -- default time scale to use after image flips or when something is wacky
...
local writeTimeScaleFieldnames     = false;
...
if S1_Movie then
   ...
   for i_delay = 1, numFrames - temporalKernelSize + 1 do
      ...
      -- buffers previous S1 internal state vector
      pv.addGroup(pvParams,
		  "S1_Buffer_" .. i_delay-1,
		  {
		     groupType                           = "ANNLayer";
		     nxScale                             = 1.0/strideX;
		     nyScale                             = 1.0/strideY;
		     ...
		     triggerOffset                       = 1; -- overwrite buffer before LCA layer updates
		     writeStep                           = writePeriodDebug;
		     initialWriteTime                    = writePeriodDebug;
		     sparseLayer                         = false;
		     updateGpu                           = false;
		     dataType                            = nil;
		     VThresh                             = -inf;
      ...
      -- clone of S1 internal state vector
      -- note: clone layers are subclasses of HyPerLayer, all piecewise linear output function variables are ignored
      pv.addGroup(pvParams,
		  "S1_Clone_" .. i_delay-1,
		  {
		     ...
		     nf                                  = S1_numFeatures;
		     phase                               = 3;
		     originalLayerName                   = "S1_" .. i_delay-1;
		     writeStep                           = writePeriodDebug;
		     initialWriteTime                    = writePeriodDebug;
		     mirrorBCflag                        = false;
		     valueBC                             = 0;
      ...
      -- prediction of S1 internal state vector at end of next display period based on  S1 internal state at end of current display period
      pv.addGroup(pvParams,
		  "S1_Predict_" .. i_delay-1,
		  pvParams["S1_Buffer_"  .. i_delay-1],
		  {
		     phase                               = 2;
		  }
      )

      -- need a predicted S1 layer with proper LCA thresholds for generating predicted reconstructions
      pv.addGroup(pvParams,
		  "S1_PredictClone_" .. i_delay-1,
		  pvParams["S1_Predict_"  .. i_delay-1],
		  {
		     phase                               = 3;
		     VThresh                             = VThresh;
		     AMin                                = 0;
      ...
      -- error in predicted S1 output
      pv.addGroup(pvParams,
		  "S1_PredictError_" .. i_delay-1,
		  pvParams["S1_Buffer_"  .. i_delay-1],
		  {
		     phase                               = 5;
		     writeStep                           = writePeriod;
   ...
   for i_frame = 1, numFrames do  
	 ...
	 pvParams["Frame" .. i_frame-1 .. "ReconS1"].triggerOffset     = 1;
      ...
      pv.addGroup(pvParams,
		  "Frame" .. i_frame-1 .. "ReconS1_Predict",
		  pvParams["Frame" .. i_frame-1 .. "ReconS1"]
      )
      pvParams["Frame" .. i_frame-1 .. "ReconS1_Predict"].triggerLayerName  = "Frame" .. i_frame-1;
      pvParams["Frame" .. i_frame-1 .. "ReconS1_Predict"].triggerBehavior   = "updateOnlyOnTrigger";
      pvParams["Frame" .. i_frame-1 .. "ReconS1_Predict"].triggerOffset     = 1;
	 ...
	 if (delta_frame >= 0 and delta_frame < temporalKernelSize) then
	    ...
	    pv.addGroup(pvParams,
			"Frame" .. i_frame-1 .. "ReconS1_PredictClone_" .. i_delay-1,
			pvParams["Frame" .. i_frame-1 .. "ReconS1_" .. i_delay-1],
			{
			   phase                               = 4;
			}
	    )
	    pvParams["Frame" .. i_frame-1 .. "ReconS1_PredictClone_" .. i_delay-1].triggerLayerName  = "Frame" .. i_frame-1;
	    pvParams["Frame" .. i_frame-1 .. "ReconS1_PredictClone_" .. i_delay-1].triggerBehavior   = "updateOnlyOnTrigger";
	    pvParams["Frame" .. i_frame-1 .. "ReconS1_PredictClone_" .. i_delay-1].triggerOffset     = 1;
...
if not S1_Movie then
   for i_frame = 1, numFrames do  
	 ...
	 if (delta_frame >= 0 and delta_frame < temporalKernelSize) then
	    ...
	    pv.addGroup(pvParams,
			"Frame" .. i_frame-1 .. "ReconS1_PredictClone_" .. i_delay-1 .. "To" .. "Frame" .. i_frame-1 .. "ReconS1_Predict",
			{
			   groupType                           = "IdentConn";
			   preLayerName                        = "Frame" .. i_frame-1 .. "ReconS1_PredictClone_" .. i_delay-1;
			   postLayerName                       = "Frame" .. i_frame-1 .. "ReconS1_Predict";
			   channelCode                         = 0;
			   delay                               = {0.000000};
			   initWeightsFile                     = nil;
			   writeStep                           = -1;
	    ...
	    if i_delay == 1 then -- the first delay layer stores the original connections
		  ...
		  pvParams["S1_" .. 0 .. "To" .. "Frame" .. delta_frame .. "ReconS1Error"].initWeightsFile     = inputPath .. "/Checkpoints/Checkpoint" .. checkpointID .. "/" ..
   ...
   for i_frame = 1, numFrames do  
	 ...
	 if (delta_frame >= 0 and delta_frame < temporalKernelSize) then
	    ...
	    pv.addGroup(pvParams,
			"S1_PredictClone_" .. i_delay-1 .. "To" .. "Frame" .. i_frame-1 .. "ReconS1_PredictClone_" .. i_delay-1,
			pvParams["S1_" .. i_delay-1 .. "To" .. "Frame" .. i_frame-1 .. "ReconS1_" .. i_delay-1],
			{
			   preLayerName                        = "S1_PredictClone_" .. i_delay-1;
			   postLayerName                       = "Frame" .. i_frame-1 .. "ReconS1_PredictClone_" .. i_delay-1;
			}
	    )
   ...
   for i_delay = 1, numFrames - temporalKernelSize + 1 do
      ...
      pv.addGroup(pvParams,
		  "S1_Clone_" .. i_delay-1 .. "To" .. "S1_Buffer_" .. i_delay-1,
		  {
		     groupType                           = "IdentConn";
		     preLayerName                        = "S1_Clone_" .. i_delay-1;
		     postLayerName                       = "S1_Buffer_" .. i_delay-1;
		     channelCode                         = 0;
		     delay                               = {0.000000};
		     initWeightsFile                     = nil;
		     writeStep                           = -1;
      ...
      pv.addGroup(pvParams,
		  "S1_Clone_" .. i_delay-1 .. "To" .. "S1_PredictError_" .. i_delay-1,
		  {
		     groupType                           = "IdentConn";
		     preLayerName                        = "S1_Clone_" .. i_delay-1;
		     postLayerName                       = "S1_PredictError_" .. i_delay-1;
		     channelCode                         = 0;
		     delay                               = {0.000000};
		     initWeightsFile                     = nil;
		     writeStep                           = -1;
      ...
      pv.addGroup(pvParams,
		  "S1_Predict" .. i_delay-1 .. "To" .. "S1_PredictError_" .. i_delay-1,
		  {
		     groupType                           = "IdentConn";
		     preLayerName                        = "S1_Predict_" .. i_delay-1;
		     postLayerName                       = "S1_PredictError_" .. i_delay-1;
		     channelCode                         = 1;
		     delay                               = {0.000000};
		     initWeightsFile                     = nil;
      ...
      pv.addGroup(pvParams,
		  "S1_Predict" .. i_delay-1 .. "To" .. "S1_PredictClone_" .. i_delay-1,
		  {
		     groupType                           = "IdentConn";
		     preLayerName                        = "S1_Predict_" .. i_delay-1;
		     postLayerName                       = "S1_PredictClone_" .. i_delay-1;
		     channelCode                         = 0;
		     delay                               = {0.000000};
		     initWeightsFile                     = nil;
		     writeStep                           = -1;
      ...
      for j_delay = 1, numFrames - temporalKernelSize + 1 do
	 --for j_delay = 1, numFrames do

	 pv.addGroup(pvParams,
		     "S1_Buffer_" .. i_delay-1 .. "To" .. "S1_PredictError_" .. j_delay-1,
		     {
			groupType                           = "HyPerConn";
			preLayerName                        = "S1_Buffer_" .. i_delay-1;
			postLayerName                       = "S1_PredictError_" .. j_delay-1;
			channelCode                         = -1;
			delay                               = {0.000000};
			numAxonalArbors                     = 1;
			plasticityFlagPredict               = plasticityFlagPredict;
			convertRateToSpikeCount             = false;
			receiveGpu                          = false;
			sharedWeights                       = true;
			weightInitType                      = "UniformRandomWeight";
			...
			selfFlag                            = false;
			combine_dW_with_W_flag              = false;
			nxp                                 = ((patchSizeX/strideX)/2)-1;
			nyp                                 = ((patchSizeY/strideY)/2)-1;
			shrinkPatches                       = false;
			normalizeMethod                     = "none";
			dWMax                               = dWMaxPredict;
			keepKernelsSynchronized             = true;
	 ...
	 if not plasticityFlagPredict then
	    pvParams["S1_Buffer_" .. i_delay-1 .. "To" .. "S1_PredictError_" .. j_delay-1].triggerLayerName    = NULL;
	    pvParams["S1_Buffer_" .. i_delay-1 .. "To" .. "S1_PredictError_" .. j_delay-1].triggerOffset       = nil;
	    pvParams["S1_Buffer_" .. i_delay-1 .. "To" .. "S1_PredictError_" .. j_delay-1].triggerBehavior     = nil;
	    pvParams["S1_Buffer_" .. i_delay-1 .. "To" .. "S1_PredictError_" .. j_delay-1].dWMax               = nil;
	 end
	 if inputPathPredict and checkpointID_predict then
	    pvParams["S1_Buffer_" .. i_delay-1 .. "To" .. "S1_PredictError_" .. j_delay-1].weightInitType      = "FileWeight";
	    pvParams["S1_Buffer_" .. i_delay-1 .. "To" .. "S1_PredictError_" .. j_delay-1].useListOfArborFiles = false;
	    pvParams["S1_Buffer_" .. i_delay-1 .. "To" .. "S1_PredictError_" .. j_delay-1].combineWeightFiles  = false;    
	    pvParams["S1_Buffer_" .. i_delay-1 .. "To" .. "S1_PredictError_" .. j_delay-1].initWeightsFile     = inputPathPredict .. "/Checkpoints/Checkpoint" .. checkpointID_predict .. "/" ..
	             "S1_Buffer_" .. i_delay-1 .. "To" .. "S1_PredictError_" .. j_delay-1 .. "_W.pvp";
	 end -- checkpointID	       
	       
	 pv.addGroup(pvParams,
		     "S1_Buffer_" .. i_delay-1 .. "To" .. "S1_Predict_" .. j_delay-1,
		     {
			groupType                           = "CloneConn";
			preLayerName                        = "S1_Buffer_" .. i_delay-1;
			postLayerName                       = "S1_Predict_" .. j_delay-1;
			channelCode                         = 0;
			delay                               = {0.000000};
			convertRateToSpikeCount             = false;
			receiveGpu                          = false;
			...
			writeCompressedCheckpoints          = false;
			selfFlag                            = false;
			originalConnName                    = "S1_Buffer_" .. i_delay-1 .. "To" .. "S1_PredictError_" .. j_delay-1,
		     }
	 )
